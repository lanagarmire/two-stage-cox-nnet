...
for (k in 1:5){
   ...
   ###########Image Hidden Layer
   W = read.csv(paste(paste('W',k,sep=''),'csv',sep='.'),header =F)
   b= read.csv(paste(paste('b',k,sep=''),'csv',sep='.'),header= F)
   W = t(W)
   b = t(b)
   test = read.csv(paste(paste('coxnet_imagetest',k,sep=''),'csv',sep='.'), header = F)
   train = read.csv(paste(paste('coxnet_imagetrain',k,sep=''),'csv',sep='.'), header = F)
   ...
   time_test = read.csv(paste(paste('coxnnet_imagetest_time',k,sep=''),'csv',sep='.'),header=F)
   sta_test = read.csv(paste(paste('coxnnet_imagetest_sta',k,sep=''),'csv',sep='.'),header=F)
   Ymatrix_test = Surv(time_test[,1],sta_test[,1])
   
   time_train = read.csv(paste(paste('coxnnet_imagetrain_time',k,sep=''),'csv',sep='.'),header=F)
   sta_train = read.csv(paste(paste('coxnnet_imagetrain_sta',k,sep=''),'csv',sep='.'),header=F)
   ...
   r = read.csv(paste(paste('coxnet_rtrain',k,sep=''),'csv',sep='.'), header = F)
   W_r = read.csv(paste(paste('Wr',k,sep=''),'csv',sep='.'),header = F)
   b_r = read.csv(paste(paste('br',k,sep=''),'csv',sep='.'),header = F)
   W_r = t(W_r)
   b_r = t(b_r)
   
   rte = read.csv(paste(paste('coxnet_rtest',k,sep=''),'csv',sep='.'),header = F)
   rte = scale(rte)
   rte[is.na(rte)] = 0
   ...
   write.csv(inte_feature_tr, paste(paste('rna_image_tr',k,sep=''),'csv',sep='.'),row.names = F,col.names = F)
   inte_feature_te = cbind(image_hidden_test,rna_hidden_te)
   inte_feature_te = scale(inte_feature_te)
   write.csv(inte_feature_te, paste(paste('rna_image_te',k,sep=''),'csv',sep='.'),row.names = F,col.names = F)
   write.csv(time_test,paste(paste('time_test',k,sep=''),'csv',sep='.'),row.names=F,col.names = F)
   write.csv(time_train,paste(paste('time_train',k,sep=''),'csv',sep='.'),row.names=F,col.names = F)
   write.csv(sta_test,paste(paste('sta_test',k,sep=''),'csv',sep='.'),row.names = F,col.names = F)
   write.csv(sta_train,paste(paste('sta_train',k,sep=''),'csv',sep='.'),row.names=F,col.names = F)
}
