...
def trainCoxMlp(x_train, ytime_train, ystatus_train, x_valid, ytime_valid, ystatus_valid, model_params = dict(), search_params = dict(), verbose=False):
    ...
    for iter in xrange(max_iter):
        ...
        if x_valid is not None:
            cost_list_valid.append(valid_negative_log_likelihood(model, x_valid, ytime_valid, ystatus_valid))
    ...
    if x_valid is not None:
        ...
        return(model, cost_iter, cost_list, cost_list_valid)
    
    else:
    
        return(model, cost_iter, cost_list) 




def valid_negative_log_likelihood(model, x_valid, ytime_valid, ystatus_valid):
    N_valid = ytime_valid.shape[0]
    R_matrix_valid = numpy.zeros([N_valid, N_valid], dtype=int)
    for i in range(N_valid):
        for j in range(N_valid):
            R_matrix_valid[i,j] = ytime_valid[j] >= ytime_valid[i]    
    theta = model.predictNewData(x_valid) 
    exp_theta = numpy.exp(theta)
    return (-numpy.sum((theta - numpy.log(numpy.sum(exp_theta * R_matrix_valid,axis=1))) * ystatus_valid))  
...
def crossValidate(x_train, ytime_train, ystatus_train, model_params = dict(),search_params = dict(),cv_params = dict(), verbose=False):
    ...
    for traincv, testcv in cv_folds:
        ...
        model, cost_iter, cost_list = trainCoxMlp(x_train = x_train_cv, ytime_train = ytime_train_cv, ystatus_train = ystatus_train_cv, x_valid=None,ytime_valid=None,ystatus_valid=None, model_params = model_params, search_params = search_params, verbose=verbose)
...
def L2Profile(x_train, ytime_train, ystatus_train, x_validation, ytime_validation, ystatus_validation, model_params = dict(),search_params = dict(),cv_params = dict(), verbose=False):
    ...
    for i in xrange(len(L2_range)):
        ...
        model, cost_iter, cost_list = trainCoxMlp(x_train = x_train, ytime_train = ytime_train, ystatus_train = ystatus_train, x_valid=None, ytime_valid=None,ystatus_valid=None, model_params = model_params, search_params = search_params, verbose=verbose)
...
def hidden_features(W,b,dat):
    """
    inputs are weights, bias of trained coxnnet model
    return a transformed data (original featrues -> hidden features)
    """
    W1 = W.T
    # normalization
    dat1 = (dat - numpy.mean(dat,axis=0)) / numpy.std(dat,axis=0)
    dat1 = numpy.nan_to_num(dat1)

    sample_size = dat1.shape[0]
    feature_size = int(math.sqrt(dat1.shape[1]))+1
    dat_hidden = numpy.ones( (sample_size,feature_size) )
    for i in range(sample_size):
        for j in range(feature_size):
            dat_hidden[i,j] = math.tanh(numpy.inner(dat1[i,:],W1[j,:])+b[j])
    return dat_hidden
...
def integration(*arg):
    """ 
    inputs are hidden features of different data types
    return a scaled column-wised integrated hidden feature data
    """
    integrated_dat = arg[0]
    for i in range(1,len(arg)):
        integrated_dat = numpy.concatenate( (integrated_dat,arg[i]), axis=1 )
    # normalization
    integrated_dat = (integrated_dat - numpy.mean(integrated_dat,axis=0)) / numpy.std(integrated_dat,axis=0)
    integrated_dat = numpy.nan_to_num(integrated_dat)
    return integrated_dat
